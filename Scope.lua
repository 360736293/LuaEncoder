local DumpStr = require "str"
local VarName = require "ran"  -- 本地变量替换
local PrintTable = require "util".PrintTable
...
local Scope = {
	...
	ObfuscateLocals = function(self, recommendedMaxLength, validNameChars)
		for i, var in pairs(self.Locals) do
			if var.Name ~= "self" then
				self:RenameLocal(var.Name, VarName())
			end
		end

		self:ObfuscateGlobals()
	end,

	-- lw new
	ObfuscateGlobals = function(self)
		-- 所有全局变量替换为env["xxx"]
		-- 排除 self, _G, env
		-- 排除形如 function AAA() ... 的函数声明
		local env = rawget(_G, "ENV") or "env"
		for i, var in pairs(self.Globals) do
			if var.Obfuscated or var.Name == env or var.Name == "self" then
				--
			else
				print("全局变量", var.Name)
				-- print(PrintTable(var))
				var.OriName = var.Name -- function AAA() 无法加密成 function _G["AAA"](), 所以需要保留原始名字
				var.Name = env.."["..DumpStr("\""..var.Name.."\"").."]"
				var.Obfuscated = 1
			end
		end
	end,
