	...
	formatExpr = function(expr, precedence)
		...
		if expr.AstType == 'VarExpr' then
			if expr.Variable then
				-- lw
				-- table.foreach(expr.Variable, print)
				out = out..(expr.Variable.OriName or expr.Variable.Name)
			else
				out = out..expr.Name
			end
		...
		elseif expr.AstType == 'StringExpr' then
			-- out = out..expr.Value.Data
			-- lw
			out = out..DumpStr(expr.Value.Data)

		elseif expr.AstType == 'BooleanExpr' then
			-- out = out..tostring(expr.Value)
			-- lw 
			out = out..RanBool(expr.Value)
		...
		elseif expr.AstType == 'MemberExpr' then
			if expr.Indexer == ":" then
				if expr.IsGlobalFunction then
					expr.Base.IsGlobalFunction = true
				end
				...
				if expr.IsGlobalFunction then
					...
					local s = DumpStr("\""..expr.Ident.Data.."\"")
					out = out..formatExpr(expr.Base).."["..s.."]"
				end
			end
	...
	local formatStatement = function(statement)
		...
		elseif statement.AstType == 'Function' then
			...
			if statement.IsLocal then
				...
				statement.Name.IsGlobalFunction = true
